...
sub sort {
  my $self=shift;
#  $self->{data} = $self->{data}->sort(&$self->{sorter});
  my $ar = $self->{data};
  $ar = sort byctime $ar;
}
...
sub fill {
 ...
 #   print "$dir : file= $_ \n";
    ...
    my $bytes = $stat[7];
    if ( $bytes < $minsize ) {
	  print "Bytes: $bytes < $minsize \n";
      unlink $full_path;
	  ++$self->{skipped};
      next;
    }
 	my $time = $stat[10];
    $self->enqueue( new MotionProxyQueueObject( $self, $full_path, $time, $bytes ) );
  }
  $self->sort();
}
